...
const transports = [
    ...
    new winston_1.default.transports.File({ filename: 'logs/error/article.log', level: 'error' }),
    new winston_1.default.transports.File({ filename: 'logs/activity/all.log', level: 'info' }),
    new winston_1.default.transports.File({ filename: 'logs/activity/article.log', level: 'info' }),
];
const articleLogger = winston_1.default.createLogger({
    level: winston_vendors_1.level(),
    levels: winston_vendors_1.levels,
    format: winston_vendors_1.format,
    transports,
});
exports.default = articleLogger;
