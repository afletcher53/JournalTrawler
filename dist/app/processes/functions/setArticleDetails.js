...
const setArticleDetails = (doi, printISSN, electronicISSN, articleData, journalId) => {
    const data = articleData.message;
    let license;
    if (articleData.message.hasOwnProperty('license')) {
        license = articleData.message.license[0]['URL'];
    }
    const { publishedPrintDate, publishedOnlineDate } = getDate_1.default(articleData);
    return new Article({
...
exports.default = setArticleDetails;
