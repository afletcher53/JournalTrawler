...
const createISSNforDOI = async (req: express.Request, res: express.Response) => {
  ...
  res.status(HttpStatusCode.OK).send({ message: StringLiterals.INTEGRITIES_START });
};

// Create a job to check the data completeness of an ISSN
const createISSNforMissing = async (req: express.Request, res: express.Response) => {
...
const updateISSN = async (req: express.Request, res: express.Response) => {
  ...
  if (req.body.issn === undefined) {
    return res.status(HttpStatusCode.CONFLICT)
      ...
      .send(serializer.serializeError(new Error(StringLiterals.ISSN_NOT_SUPPLIED)));
  }
  req.body.issn = req.body.issn.replace(/[\u200c\u200b]/g, '');
...
const findOne = (req, res) => {
  ...
  Integrity.findById(req.params.id)
    .then((data) => {
      if (!data) {
        res.status(HttpStatusCode.NOT_FOUND).send(
          { message: StringLiterals.INTEGRITY_NOT_FOUND });
      } else {
        res.send(serializer.serialize('integrity', data));
      }
    })
    .catch((err) => {
      res.status(HttpStatusCode.INTERNAL_SERVER_ERROR).send(
        { message: StringLiterals.INTEGRITY_ERROR_FIND });
    });
